...
struct ConstraintData {
    ...
    ConstraintData(const EuclideanVector &position, int axonId, int growthStep) : m_position(position),
                                                                                  m_axonId(axonId),
                                                                                  m_growthStep(growthStep) {}

    bool operator==(const ConstraintData &constraintData) const {
        return m_position == constraintData.m_position && m_growthStep == constraintData.m_growthStep &&
               m_axonId == constraintData.m_axonId;
    };
};
...
public:
    ...
    bool checkForConstraintAndAdd(const EuclideanVector &start, const EuclideanVector &growthVector, int axonIdentifier,
                                  int growthStep);
    ...
    bool operator==(const ConstraintManager &constraintManager) const {
        return m_centerMap == constraintManager.m_centerMap && m_distanceLimit == constraintManager.m_distanceLimit &&
               m_exteriorLimit == constraintManager.m_exteriorLimit;
    };
