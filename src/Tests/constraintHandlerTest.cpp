...
TEST_F(ConstraintHandlerTest, checkOccupiedSpace) {
    ...
    EXPECT_TRUE(
            m_constraintHandler->checkForConstraint(std::vector<EuclideanVector>({EuclideanVector({1.009, 1, 1})})));
    EXPECT_TRUE(
            m_constraintHandler->checkForConstraint(std::vector<EuclideanVector>({EuclideanVector({1, 1.009, 1})})));
    EXPECT_TRUE(
            m_constraintHandler->checkForConstraint(std::vector<EuclideanVector>({EuclideanVector({1, 1, 1.009})})));
    EXPECT_TRUE(
            m_constraintHandler->checkForConstraint(
                    std::vector<EuclideanVector>({EuclideanVector({1.005, 1.005, 1})})));
    EXPECT_TRUE(
            m_constraintHandler->checkForConstraint(
                    std::vector<EuclideanVector>({EuclideanVector({1.005, 1, 1.005})})));
    EXPECT_TRUE(
            m_constraintHandler->checkForConstraint(
                    std::vector<EuclideanVector>({EuclideanVector({1, 1.005, 1.005})})));
