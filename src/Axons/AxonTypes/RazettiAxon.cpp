...
RazettiAxon::RazettiAxon(int numberOfGrowthStepsEachTimeStep, int numberOfRetractions, double alpha, double beta,
        ...
        : BaseAxon(startPosition, constraintManager),
          ...
          m_numberOfRetractions(numberOfRetractions), m_alpha(alpha), m_beta(beta),
...
RazettiAxon::RazettiAxon(int numberOfGrowthStepsEachTimeStep, int numberOfRetractions, double alpha, double beta,
                ...
                stepLength) { m_growthAngles.push_back(std::make_pair<double, double>(0, 0));
...
void RazettiAxon::grow() {
    ...
    while (growthStep < m_GrowthStepsEachTimeStep &&
        ...
        centersCurrentTimeStep.push_back(
                HelperFunctions::createCoveringCenters(tipPositionsCurrentTimeStep.back(), growthVector, parameters.minDistance));
...
EuclideanVector RazettiAxon::sampleVector() {
    ...
    auto newTheta0 = std::normal_distribution<>(expectation_0, variation_0)(parameters.generator);

    auto expectation_1 = (m_alpha / (m_alpha + m_beta)) * m_growthAngles.back().second;
    auto variation_1 = 1 / sqrt(2 * (m_alpha + m_beta));
    auto newTheta1 = std::normal_distribution<>(expectation_1, variation_1)(parameters.generator);
...
RazettiAxon::RazettiAxon(int numberOfGrowthStepsEachTimeStep, int numberOfRetractions, double alpha, double beta,
                         ...
                         std::pair<double, double> startingAngles,
                         ConstraintManagerHandle constraintManager) : BaseAxon(startPosition,
